package com.smart_dispatch.driver.ui;

import com.smart_dispatch.driver.model.DispatchAlert;
import com.smart_dispatch.driver.service.DriverStompService;
...
public class DriverView extends VerticalLayout {
    ...
    private final DriverStompService stompService;
    ...
    public DriverView(DriverStompService stompService) {
        ...
        // Top Bar
        topBar = new Div();
        topBar.setText("Establishing secure link to HQ...");
        topBar.getStyle()
    ...
    protected void onAttach(AttachEvent attachEvent) {
        ...
        statusListener = status -> {
            ui.access(() -> {
                topBar.setText(status);
                if (status.contains("ACTIVE")) {
                    topBar.getStyle().set("color", "#00ff00");
                } else if (status.contains("LOST")) {
                    topBar.getStyle().set("color", "#ff4444");
                } else {
                    topBar.getStyle().set("color", "#ffaa00");
                }
            });
        };

        stompService.addAlertListener(alertListener);
        stompService.addStatusListener(statusListener);
    }

    @Override
    protected void onDetach(DetachEvent detachEvent) {
        if (alertListener != null) {
            stompService.removeAlertListener(alertListener);
        }
        if (statusListener != null) {
            stompService.removeStatusListener(statusListener);
        }
