package com.smart_dispatch.paramedic.ui;

import com.smart_dispatch.paramedic.service.EmergencyDispatchClient;
import com.smart_dispatch.paramedic.service.EmergencyDispatchClient.DispatchResult;
...
public class ParamedicView extends VerticalLayout {
    ...
    public ParamedicView(EmergencyDispatchClient dispatchClient) {
        setSpacing(true);
        setPadding(true);
        ...
        dispatchBtn.addClickListener(e -> {
            ...
            DispatchResult result = dispatchClient.sendDispatch(
            ...
            if (result.success()) {
                showModal("✅ Match Found", result.message() + "\n\nTransmitting route to Driver...", "#00ff00");
            } else {
                showModal("❌ Dispatch Failed", result.message() != null && !result.message().isBlank() ? result.message() : "No eligible hospitals available.", "#ff4444");
            }
